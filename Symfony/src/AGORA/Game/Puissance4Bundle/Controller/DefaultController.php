...
class DefaultController extends Controller {
	...
	public function indexAction() {
		return $this->render('AGORAGamePuissance4Bundle:Default:index.html.twig');
	}
	...
	public function createGameAction() {
		...
		//On récupère l'objet Service (Singleton), qui nous permettra de faire le lien entre ce contrôleur et Puissance4GameModel.
		$service = $this->container->get('agora_game.puissance4');

		//Création d'une partie de Puissance4. On récupère l'identifiant de la partie (Table : Puissance4Game).
		$gameId = $service->createPuissance4Game($_POST['lobbyName'], $user);
	...
	/**
	 * Fonction permettant de rejoindre la partie de Puissance4 identifiée par $gameId.
	 */
	public function joinGameAction($gameId) {
		...
		//On récupére l'objet Service (Singleton), qui nous permettra de faire le lien entre ce contrôleur et Puissance4GameModel.
		$service = $this->container->get('agora_game.puissance4');
	...
	/**
	 * Fonction permettant de rejoindre la table de jeu du Puissance4 identifiée par $gameId.
	 */
	public function PlayAction($gameId) {
		...
		$service = $this->container->get('agora_game.puissance4');
		$player = $service->getPlayerEntityPuissance4($user->getId(), $gameId);
		if ($player == null) {
			return $this->redirect($this->generateUrl('agora_platform_joingame'));
		}
	...
	/**
	 * Fonction permettant de quitter la partie de Puissance4 identifiée par $gameId si elle n'a pas encore commencée.
	 */
