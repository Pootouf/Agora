...
/**
 * Implémente MessageComponentInterface, une interface de Ratchet pour la gestion
 * des évenements d'une WebSocket : onOpen, onMessage, onClose et onError.
 */
class MorpionSocket implements MessageComponentInterface {
    ...
    function __construct(Container $container) {
        ...
        $this->service = $container->get('agora_game.morpion');
        $this->logService = $container->get('agora_game.morpionLog');
    }
    ...
    public function onMessage(ConnectionInterface $from, $msg) {
        ...
        if ($content->type == "connect") {
            ...
            if ($playerIdFrom == $this->service->getCurrentPlayerID($gameId)) {
                $symbole = $this->service->getPlayerEntityMorpionFromPlayerId($playerIdFrom, $gameId)->getSymbole();

                $this->service->jouerCase($content->case, $gameId, $playerIdFrom, $symbole);
                $this->resfreshPlayers($gameId);

                $game = $this->service->getMorpionGameFromGame($gameId);
                if ($game->getState() == "finished") {
                    $board = explode(";", $this->service->getGame($gameId)->getBoard());
            ...
            foreach ($this->connectionStorage->getAllConnections($gameId) as $c) {
                ...
                if ($TableauEnvoye["action"] == 'begin' || $TableauEnvoye["action"] == 'win') {
                    ...
                    $playerSymbol = $this->service->getPlayerEntityMorpionFromPlayerId($playerId, $gameId)->getSymbole();
